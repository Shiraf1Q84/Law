...
def upload_to_pinecone(chunks, embeddings, index):
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        vectors = [{"id": id, "values": emb.tolist(), "metadata": meta} 
                   for id, emb, meta in zip(ids, embeddings[i:i_end], metadatas)]
        ...
        if uploaded_files:
            for uploaded_file in uploaded_files:
                ...
                with st.spinner(f'Processing {uploaded_file.name}...'):
                    ...
                    # Pineconeにアップロード
                    upload_to_pinecone(chunks, embeddings, st.session_state.index)
